...
#Y = Y + np.sqrt(1e-2**2) * rng.normal(Y, math.sqrt((1e-2)**2), (Y.shape[0],1)).astype(dtype)
#
...
data = Namespace()
#data.x = np.linspace(0,max_val, 1, dtype=dtype)
...
for i in range(1):
    ...
    #x_test = np.linspace(0,max_val, n_acqopt, dtype=dtype)
    ...
    ax.plot(X, Y, '.', color='#C0C0C0', markersize=8)
    # -- plot observations
    for x in data.x:
        ax.scatter(x[0], f(x[0]), color=(0, 0, 0, 1), s=80)
    # -- plot x_next
    ax.scatter(x_next[0], f(x_next[0]), color='deeppink', s=80, zorder=10)
    # -- plot estimated output
    for out in acqfn.output_list:
        out_arr = np.array(out.x)
        y_out_arr = [f(x[0]) for x in out_arr]
        ax.plot(
            out_arr[:, 0], y_out_arr, 's', markersize=8, color='blue', alpha=0.02
        )
